...
def load_whisper_model(model_size: str, dry_run: bool = False):
    ...
    try:
        ...
        raise ValueError(
            f"Failed to load Whisper model '{model_path}'. Check model size. Error: {e}"
        )
...
def train_whisper_model(
    ...
    num_epochs: int = 3,
    dry_run: bool = False,
    wandb_project: str = "whisper-default",
    wandb_entity: str = "default-entity",
    ...
    input_features = [
        processor(a, sampling_rate=sr, return_tensors="pt").input_features[0]
        for a, sr in zip(audio_list, sr_list)
    ]
    with processor.as_target_processor():
        labels = [
            processor(t, return_tensors="pt").input_ids[0] for t in ds["sentence"]
        ]
    ...
    def_metrics = lambda pred: {
        "wer": wer.compute(
            predictions=processor.batch_decode(
                torch.argmax(torch.tensor(pred.predictions), dim=-1),
